...
vector <Route> repair_one(ProcessedInput* input, double peso_TW, double inter_stop_weight, double end_day, vector<Route>& routes_destroyed, vector <Passenger>& passengers_removed) {
	...
	for (Passenger& p : passengers_removed) {
		...
		for (int r = 0; r < (int)routes_destroyed.size(); r++) {
			...
			if (route_ptr->primo_pass == false) {
				...
				double cost_route_before = cost_single_route(input, peso_TW, inter_stop_weight, routes_destroyed[r]);
...
inline vector<Route> two_regret_repair_aggragati(ProcessedInput* input, double peso_TW, double inter_stop_weight, double end_day, vector<Route>& routes_destroyed, vector <Passenger>& passengers_removed) {
...
vector <Route> repair_forbidden(ProcessedInput* input, double peso_TW, double peso_intermediate_stop, double end_day, vector<Route>& routes_destroyed, vector <Passenger>& passengers_removed) {
