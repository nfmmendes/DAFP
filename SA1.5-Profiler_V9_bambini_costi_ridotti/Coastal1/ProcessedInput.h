...
{
public:
	ProcessedInput(std::map <int, Airplane>& map_airplane, map<unsigned int, Airstrip>& map_airstrip, double2DVector& from_to, double2DVector& location_fuel, double3DVector& fuel_consumption);

	map<int, Airplane>& get_map_airplane();
	const map<unsigned int, Airstrip>& get_map_airstrip() const;
	...
	void set_map_airstrip(const map<unsigned int, Airstrip>& p_map_airstrip);
