...
int main(int argc, char* argv[]) {
	...
	int number_heuristic_iterations = 0;
	...
	do {
		...
		if (iter_first_do == 0) {
			...
			double heuristic_choice = static_cast<double>(rand()) / RAND_MAX;
			int npass = 0;
			do {
				...
				if (heuristic_choice < Accumulated(0, Weigth_heuristic)) {
					start_solution_route = costructive_first_fase(&input, penalty_weights, airplanes, end_day, passengers, number_of_aircraft);
					choosen_heuristic = 0;
				}
				else {
					start_solution_route = sequential_costructive_first_fase(&input, penalty_weights, end_day, passengers, number_of_aircraft);
					choosen_heuristic = 1;
				}
				for (Route& r : start_solution_route) 
					npass += static_cast<int>(r.get_passengers().size());

				cout << "numero di passeggeri " << npass << endl;
				cout << "passengers.size()= " << passengers.size() << endl;
				number_heuristic_iterations++;
		...
		do {
			...
			if (!solution_rebuilt.empty()) {
				...
				//*************************************per scrittura su .txt per grafico**************************************
				if (numero_sa%1000 == 0 && numero_sa >= 1000 && numero_sa <= 60000){
					final_time = difftime(time(NULL), time_now);
					outfile << "NumeroSA:;" << numero_sa << ";" << best_solution << ";tempo:;" << to_string(final_time) << endl;
				}
				
				if (cost_objectiveFunction_second_fase_after_rebuilt < start_solution) {
					
					start_solution_route.clear();
					for (Route& so : solution_rebuilt) 
						start_solution_route.push_back(so);
				...
				cout << "Numero iterazioni ---> " << numero_sa << endl;
			}
			else {
				failed_iterations++;
			}
		...
		} while (iteration <= iteration_max && T > Tmin && numero_sa <= 20000);

		if (iter_first_do > 4) 
			cout << " Iterazioni senza migliorare " << iteration << " Temperaruta: " << T << endl;
		final_time = difftime(time(NULL), time_now);
		outfile << "soluzione prima del modello:;" << numero_sa << ";" << best_solution << ";tempo:; " << to_string(final_time) << endl;
		...
		iter_first_do++;
		cout << "*****************************************************************************************************************" << endl;
		cout << " ************************ ITERATION WHILE: " << iter_first_do << " OF: " << iterMAX_FirstDo << "******************" << endl;
		cout << "*****************************************************************************************************************" << endl;
		...
		delete model; 
	} while ((iter_first_do <= iterMAX_FirstDo));
	...
	cout << "Time at the end: " << final_time << endl;
	cout << "Time model: " << time_spent_model << endl;
	cout << "Numero SA= " << numero_sa << endl;
	cout << "Iterazioni Fallite= " << failed_iterations << endl;
	cout << "number of iteration in the heuristic: " << number_heuristic_iterations << endl;
