...
namespace heuristic_costructive_first_fase_namespace {
	...
	inline void run_situation_2(ProcessedInput* input, Passenger& p, Route* route)
	{
		...
		if (origin->fuel) {
			route->addPlace(p.origin, true, airplane->max_fuel, 0.0, 1, p.early_departure, p.early_departure + origin->ground_time);
			...
			route->addPlace(p.origin, false, fuel_before - fuel_consumed, 0.0, p.capacity, p.early_departure, p.early_departure + origin->ground_time);
	...
	for (auto p : passengers) {
		...
		for (auto r : solution) {
			...
			if (r.primo_pass == false) {
				...
				if (!FROM.empty()) {
					...
					if (r.get_airstrips()[r.index - 1] != p.destination) {
						for (auto from : FROM) {
							...
							double travel_time = (from_to[previous_airstrip][p.destination] / airplane->speed) * 60;
				...
				double travel_time = 60.0*from_to[previous_airstrip][p.origin] / map_airplane[r.aircraft_code].speed;
...
void run_situation_2(ProcessedInput* input, vector<Passenger>& passengers, Route& r, const int& i_best_passenger)
{
	...
	if (map_airstrip[origin].fuel) {
		r.addPlace(origin, map_airstrip[origin].fuel, 
				   map_airplane[r.aircraft_code].max_fuel, 0.0, best_passenger.capacity, best_passenger.early_departure, 
		...
		r.addPlace(origin, map_airstrip[origin].fuel, 
				   fuel_before - fuel_consumed, 0.0, best_passenger.capacity, best_passenger.early_departure, 
				   best_passenger.early_departure + map_airstrip[origin].ground_time);
	...
	if (map_airstrip[destination].fuel) {
		r.addPlace(destination, true, map_airplane[r.aircraft_code].max_fuel,
		...
		r.addPlace(destination, false, fuel_before - fuel_consumed, 
			      0.0, 0, time, time + map_airstrip[destination].ground_time);
		r.get_weight_at(r.index - 1) = r.get_weights()[r.index - 2] + best_passenger.weight + fuel_consumed;
			...
			for (int p = 0; p < (int)passengers.size(); p++) {
				...
				if (r.primo_pass == false) {
					if (previous_airstrip == origin) {
						...
						if (best_cost > cost && f_after_primo_tratto >= map_airplane[r.aircraft_code].min_fuel) {
					...
					if (FROM.size() != 0) {
						...
						if (r.get_airstrips()[r.index - 1] != passengers[p].destination) {
							for (auto from : FROM) {
								...
								double fuel_consumed = fuel_consumption[r.aircraft_code][previous_airstrip][destination];
								...
								t_arr_arrival = r.get_arrivals()[r.index - 1] + (travel_time * 60) + map_airstrip[previous_airstrip].ground_time;
								...
								if (fuel_ok && best_cost > cost && t_arr_arrival <= end_day) {
									...
									auto fuel_needed = (map_airplane[r.aircraft_code].min_fuel + location_fuel[r.aircraft_code][destination]);
...
map<int, int> Compute_WorstNode(double peso_TW, double stop_weight, Route& route, map<int, Airstrip>& map_airstrip, double2DVector& from_to) {
...
namespace heuristic_costructive_first_fase_secIter_namespace
{
	...
	void run_situation_2(ProcessedInput* input, vector<Route>& solution, Passenger& p, int& best_route)
	{
		...
		auto previous_airstrip = route->get_airstrips()[route->index - 1];
		double travel_time = 60*(from_to[previous_airstrip][p.origin] / airplane->speed);
		
		route->primo_pass = true;
		route->get_arrival_at(route->index - 1) = p.early_departure -  travel_time - map_airstrip[previous_airstrip].ground_time;
		route->get_departure_at(route->index - 1) = p.early_departure - 60*from_to[previous_airstrip][p.origin] / airplane->speed;
		...
		if (origin->fuel) {
			route->addPlace(p.origin, true, airplane->max_fuel, 0.0, p.capacity, 
		...
		p.solution_from = route->index - 1;
		double time = p.early_departure + origin->ground_time + 60.0*from_to[p.origin][p.destination] / airplane->speed;
	...
	void run_situation_3(ProcessedInput* input, vector<Route>& solution, Passenger& p, int& best_route)
	{
		...
		auto previous_airstrip = route->get_airstrips()[route->index - 1];
		double time_to_go = 60.0*from_to[previous_airstrip][p.origin] / airplane->speed;
	...
	void run_situation_5(ProcessedInput* input, vector<Route>& solution, Passenger& p, int& best_route, int& best_from)
	{
		...
		int place_1 = route->get_airstrips()[route->index - 1];
		double aircraft_speed = map_airplane[route->aircraft_code].speed;
		double arrival_time = route->get_departures()[route->index - 1] + ((from_to[place_1][p.destination] / aircraft_speed) * 60);
		...
		for (auto& r : solution) {
			...
			if (r.primo_pass == false) {
					...
					if (r.get_airstrips()[r.index - 1] != p.destination) {
						for (auto from : FROM) {
							...
							if (capacity_satisfy && (r.index - from) <= p.stop) {
								...
								double time_to_go = 60.0*from_to[previous_airstrip][p.destination]/airplane->speed;
				...
				if (r.get_airstrips()[r.index - 1] != p.origin) {
					...
					double time_to_go = 60*(from_to[previous_airstrip][p.origin] / map_airplane[r.aircraft_code].speed); 
					double t_arr_departure = r.get_arrivals()[r.index - 1] + map_airstrip[previous_airstrip].ground_time + time_to_go;
					double TW_departure = max(0.0,  p.early_departure - t_arr_departure) + max(0.0, t_arr_departure - p.late_departure);
					
					time_to_go = 60.0*(from_to[p.origin][p.destination] / map_airplane[r.aircraft_code].speed);
